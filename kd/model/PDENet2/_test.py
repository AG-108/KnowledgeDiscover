...
with torch.no_grad():
    ...
    for bidx in range(len(blocks)):
        ...
        for stepnum in range(max_blocks):
            ...
            var = uvars[stepnum]
            err = (err.sum(dim=1)/var.sum(dim=1)).detach().cpu().numpy()
            errs[bidx, :, stepnum] = err
            updateerr = ((ut_infe-ut_obs)**2).mean(dim=-1, keepdim=False).mean(dim=-1, keepdim=False)
            updateerr = (updateerr.sum(dim=1)/updatevars[stepnum].sum(dim=1)).detach().cpu().numpy()
    ...
    uvars = torch.stack(uvars, dim=-1).sum(dim=1).detach().cpu().numpy()
    updatevars = torch.stack(updatevars, dim=-1).sum(dim=1).detach().cpu().numpy()
