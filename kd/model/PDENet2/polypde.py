...
class POLYPDE2D(torch.nn.Module):
    ...
    def RightHandItems(self, u):
        ...
        if self.scheme.upper() == 'UPWIND':
            with torch.enable_grad(): # convection direction and update U01[...],U10[...]
                ...
                U01Tmp = []; U10Tmp = []
                for k in range(self.channel_num):
                    U01Tmp.append(U01[k].detach().clone().requires_grad_(True))
                    U10Tmp.append(U10[k].detach().clone().requires_grad_(True))
    ...
    def multistep(self, init, stepnum):
        if isinstance(stepnum, float):
            if not stepnum.is_integer():
                raise ValueError(f"stepnum 必须是整数，但得到 {stepnum}")
            stepnum = int(stepnum)
        if init.dim() == 2:
