...
def main():
    ...
    supportPoints = None
    # support_points = np.linspace(xRange[0],xRange[1],number_of_points[1])
    # support_points = [[np.round(p,decimals)] for p in support_points]
    # support_points = [[np.round(p,decimals), np.round(p,decimals)] for p in support_points]
    # support_points = [[np.round(p,decimals) for i in range(numVars[0])] for p in support_points]

    supportPointsTest = None
    # support_points = None # uncomment this line if you don't want to use support points
    # support_points_test = np.linspace(xRange[0],xRange[1],number_of_points[1])
    # support_points_test = [[np.round(p,decimals) for i in range(numVars[0])] for p in support_points_test]
    ...
    for i, nv in enumerate(numVars):
        ...
        # p = mp.Process(target=processData, args=(num_samples, nv, decimals, template, data_path, fileID, time, support_points, number_of_points,seed,))
